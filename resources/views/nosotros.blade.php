...
<body class="antialiased font-sans bg-[#F0F4FF]">
    ...
    <script type="text/babel">
        ...
        @php
            ...
            $semaforoContent = file_get_contents(base_path('assets/semaforo.html'));
            $semaforoContent = str_replace('__CSRF_TOKEN__', csrf_token(), $semaforoContent);
        @endphp
        const apiKey = "{{ $aiKey }}";
        const GEMINI_MODEL = "{{ $aiModel ?? 'gemini-2.0-flash-exp' }}"; 
        const semaforoHtml = {!! json_encode($semaforoContent) !!};
        @verbatim
                ...
                <div className={`min-h-screen ${activeTheme.bg} font-sans ${activeTheme.text} transition-colors duration-500 selection:bg-blue-200`}>
                    ...
                    <section id="ia-tools" className={`py-24 ${activeTheme.toolBg} transition-colors`}>
                        <div className="container mx-auto px-6">
                            ...
                            <div className="w-full h-[1000px] overflow-hidden rounded-[40px] shadow-lg border-4 border-white/20">
                                <iframe srcDoc={semaforoHtml} className="w-full h-full border-0" title="Semaforo"></iframe>
                            </div>
